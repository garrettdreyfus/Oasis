...
for(var i=0;i<10;i++)
{
	animals.push( new Wolf(Math.floor((Math.random()*100)+1),Math.floor((Math.random()*100)+1),100,0,0,0,1,10));	
} 
for(var i=0;i<10;i++)
{
	animals.push( new Bunny(Math.floor((Math.random()*100)+1),Math.floor((Math.random()*100)+1),100,0,0,0,1,10));	
} 
...
{
	dead.push(animals);
	animals = animals.splice(animals.indexOf(animal),-1);
}
...
var drawAnimals = function(animals)
{
	...
	for(var i=0;i<animals.length;i++)
	{
		...
		if(animals[i].species == 'Bunny'){context.fillStyle="#ffff00"}
    		context.fillRect(Math.floor(animals[i].x / 5) *25 ,Math.floor(animals[i].y / 5) *25 ,5,5);
	}
